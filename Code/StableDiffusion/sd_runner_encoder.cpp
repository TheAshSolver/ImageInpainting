...
int main(int argc, char** argv) {
    ...
    // Text Encoder Setup
    textEncoder.addInputTensor(0, "tokens", QNN_DATATYPE_INT_32, {1, 77});
    textEncoder.addOutputTensor(0, "text_embedding", QNN_DATATYPE_UFIXED_POINT_16, {1, 77, 768}, TE_OUT_SCALE, TE_OUT_ZERO_POINT);
    ...
    // UNet Setup
    unet.addInputTensor(0, "timestep", QNN_DATATYPE_UFIXED_POINT_16, {1, 1}, UNET_TIMESTEP_SCALE, UNET_TIMESTEP_ZP);
    unet.addInputTensor(0, "latent", QNN_DATATYPE_UFIXED_POINT_16, {1, 64, 64, 4}, UNET_LATENT_IN_SCALE, UNET_LATENT_IN_ZP);
    unet.addInputTensor(0, "text_emb", QNN_DATATYPE_UFIXED_POINT_16, {1, 77, 768}, UNET_TEXT_EMB_SCALE, UNET_TEXT_EMB_ZP);
    unet.addOutputTensor(0, "output_latent", QNN_DATATYPE_UFIXED_POINT_16, {1, 64, 64, 4}, UNET_OUT_SCALE, UNET_OUT_ZP);
    ...
    // VAE Decoder Setup
    vaeDecoder.addInputTensor(0, "latent", QNN_DATATYPE_UFIXED_POINT_16, {1, 64, 64, 4}, VAE_LATENT_IN_SCALE, VAE_LATENT_IN_ZP);
    vaeDecoder.addOutputTensor(0, "image", QNN_DATATYPE_UFIXED_POINT_16, {1, 512, 512, 3}, VAE_IMAGE_OUT_SCALE, VAE_IMAGE_OUT_ZP);
    ...
    vaeEncoder.addInputTensor(0, "image_input", QNN_DATATYPE_UFIXED_POINT_16, {1, 512, 512, 3}, VAE_ENC_IN_SCALE, VAE_ENC_IN_ZP);
    vaeEncoder.addOutputTensor(0, "latent_output", QNN_DATATYPE_UFIXED_POINT_16, {1, 64, 64, 4}, VAE_ENC_OUT_SCALE, VAE_ENC_OUT_ZP);
    ...
    for (size_t i = 0; i < imagePixelCount; ++i) {
        float pixel = std::clamp(imageBuf[i], -1.0f, 1.0f);
